...
function loggedIn(req, res, next) {
    ...
    connection.query(
        "SELECT authToken, tokenDeath FROM auth WHERE user_id=?",
        req.cookies.user_id,
        (err, authToken) => {
            if (err) {
                console.error(err);
            ...
            if (
                authToken.length &&
                authToken[0].authToken == req.cookies.auth_token &&
                authToken[0].tokenDeath > 0
            ) {
                connection.query(
                    "UPDATE auth SET tokenDeath=2629800000 WHERE user_id=?",
                    [req.cookies.user_id],
                    (err) => {
                        if (err) {
                            console.error(err);
...
function default_login_check(req, res, next) {
    if (!req.cookies.user_id || !req.cookies.auth_token) return next();
...
function leaderboardSort(lb, low, high) {
    ...
    for (let j = low; j < high; j++) {
        ...
        if (
            lb[j].wins / lb[j].totalGames >
            lb[high].wins / lb[high].totalGames
        ) {
            findpivot++;
...
function getPropertyValue(currentProperty, obj) {
    if (currentProperty <= 3)
        return currentProperty == 3
            ? Math.round(obj[properties[currentProperty]])
            : obj[properties[currentProperty]];

    if (currentProperty == 4) {
        if (obj.wins == 0 && obj.totalGames > 0) return "0%";
        else if (obj.totalGames == 0) return "No games";
...
app.get("/l", loggedIn, (req, res, next) => {
    connection.query(
        `SELECT id, username, darkmode, game.*, streak.* FROM user INNER JOIN game
		ON user.id=game.user_id INNER JOIN streak ON
		user.id=streak.user_id WHERE user.id=?`,
        req.cookies.user_id,
        async (err, user_data) => {
            ...
            connection.query(
                `SELECT username, currentScore, bestScore, bestBlock, wins, averageScore,
			totalGames FROM game INNER JOIN user ON user.id=game.user_id ORDER BY ${
                propertiesSQL[user_data[0].leaderboardProperty]
            }`,
                (err, users) => {
                    if (err || !users) {
                        return next({
                    ...
                    for (let i = 0; i < users.length; i++) {
                        if (i >= 20) {
                            if (
                                !onLeaderboard &&
                                users[i].username == u_dat.username
                            ) {
                        ...
                        newU.rank = i + 1;
                        newU.score = getPropertyValue(
                            u_dat.leaderboardProperty,
                            u
                        );

                        newU.username =
                            u.username +
                            (u.username == u_dat.username
                                ? " <span id='leaderboard-personal' class='is-taken'>(you)</span>"
                                : "");
                    ...
                    res.render("index", {
                        ...
                        LEADERBOARD_PROPERTY:
                            propertiesUI[u_dat.leaderboardProperty],
                        LARGE_LEADERBOARD_PROPERTY:
                            u_dat.leaderboardProperty == 3 ? true : false,
                        LEADERBOARD: leaderboardIndex,

                        USER_SPECIAL: onLeaderboard ? false : true,
                        USER_RANK: user_special_rank,
                        USER_SPECIAL_SCORE: getPropertyValue(
                            u_dat.leaderboardProperty,
                            u_dat
                        ),
...
app.get("/leaderboard/:property?", (req, res, _next) => {
    ...
    connection.query(
        `SELECT id, username, darkmode FROM user WHERE user.id=?`,
        req.cookies.user_id ?? 0,
        async (err, user_data) => {
			...
			totalGames FROM game INNER JOIN user ON user.id=game.user_id ORDER BY ${propertiesSQL[leaderboardProperty]}`,
                (err, users) => {
                    ...
                    res.render("leaderboard", {
                        ...
                        LARGE_LEADERBOARD_PROPERTY:
                            leaderboardProperty == 3 ? true : false,
                        LEADERBOARD: leaderboardIndex,
...
app.get("/user/:username/", loggedIn, (req, res, next) => {
    connection.query(
        "SELECT id, currentScore, bestScore FROM game INNER JOIN user ON game.user_id=user.id WHERE username=?",
        req.params.username,
        (err, user_data) => {
            if (err) {
                return next({
            ...
            connection.query(
                "SELECT game_id, endTime FROM board_history WHERE user_id=? AND (YEARWEEK(endTime) = YEARWEEK(NOW() - INTERVAL 1 WEEK) OR YEARWEEK(endTime) = YEARWEEK(NOW())) ORDER BY endTime ASC",
                user_data[0].id,
                (err, boards) => {
                    ...
                    for (let b = 0; b < boards.length; b++) {
                        ...
                        maxTotalGames =
                            totalGames[bTimeDate] > maxTotalGames
                                ? totalGames[bTimeDate]
                                : maxTotalGames;
                    }
...
app.get("/leaderboard-property/:lbprop", loggedIn, (req, res, next) => {
    ...
    if (propNum == undefined) return res.send("1");

    connection.query(
        "UPDATE game SET leaderboardProperty=? WHERE user_id=?",
        [propNum, req.cookies.user_id],
        (err) => {
            if (err) {
                return next({
                    err,
...
app.get("/updated-leaderboard", (req, res, next) => {
    connection.query(
        "SELECT username, leaderboardOpen, leaderboardProperty FROM user INNER JOIN game ON user.id=game.user_id WHERE id=?",
        req.cookies.user_id ?? 0,
        (err, user_data) => {
            if (err) {
                return next({
            ...
            connection.query(
                `SELECT username, bestScore, bestBlock, wins, averageScore, totalGames FROM game
			INNER JOIN user ON user.id=game.user_id ORDER BY ${
                propertiesSQL[user_data[0].leaderboardProperty ?? 0]
            }`,
                (err, users) => {
                    if (err || !users) {
                        return next({
                    ...
                    for (let i = 0; i < users.length; i++) {
                        if (i >= 20) {
                            if (
                                ...
                                lowUser.score = getPropertyValue(
                                    u_dat.leaderboardProperty,
                                    users[i]
                                );
...
app.get("/toggle-leaderboard/:onoff", loggedIn, (req, res, next) => {
    ...
    connection.query(
        "UPDATE game SET leaderboardOpen=? WHERE user_id=?",
        [toggleStatus, req.cookies.user_id],
        (err) => {
            if (err) {
                return next({
                    err,
...
app.get("/darkmode/:onoff", loggedIn, (req, res, next) => {
    ...
    connection.query(
        "UPDATE user SET darkmode=? WHERE id=?",
        [toggleStatus, req.cookies.user_id],
        (err) => {
            if (err) {
                return next({
...
app.get("/previous-board", loggedIn, (req, res, next) => {
    connection.query(
        "SELECT wholeBoard FROM current_board WHERE user_id=?",
        req.cookies.user_id,
        (err, board) => {
...
app.post("/new-game", loggedIn, async (req, res, next) => {
    if (!req.body.piece) return res.end("1");

    let pieces = req.body.piece;

    if (pieces.length != 2) return res.end("1");

    if (
        !allowed_new_blocks[pieces[0].num] ||
    ...
    try {
        await Promise.all([
            new Promise((resolve, reject) => {
                connection.query(
                    `UPDATE current_board SET wholeBoard=?, startTime=? WHERE user_id=?`,
                    [JSON.stringify(newBoard), new Date(), req.cookies.user_id],
                    (complete) => {
            ...
            new Promise((resolve, reject) => {
                connection.query(
                    `UPDATE game SET currentScore=0 WHERE user_id=?`,
                    [req.cookies.user_id],
                    (complete) => {
                        if (complete) reject(complete);
                        resolve();
...
app.post("/move-game", loggedIn, (req, res, next) => {
    ...
    connection.query(
        `SELECT currentScore, bestScore, averageScore, bestBlock, totalGames, game_id, wholeBoard,
        ...
        (err, gameState) => {
            ...
            for (let play = 0; play < move.length; play++) {
                let boardDiffs = moveBoard[move[play].move](
                    game.wholeBoard,
                    game.currBestBlock
                );
                ...
                game.bestBlock =
                    game.bestBlock < boardDiffs.bestBlock
                        ? boardDiffs.bestBlock
                        : game.bestBlock;
                game.bestScore =
                ...
                if (newPiece) {
                    if (
                        game.wholeBoard[newPiece.x][newPiece.y] ||
                        !allowed_new_blocks[newPiece.num]
                    ) {
                        console.error(
                            "Invalid Block place or illegal Block piece"
                        );
                        return res.send("2");
                    }
            ...
            connection.query(
                "UPDATE game SET currentScore=?, bestScore=?, bestBlock=? WHERE user_id=?",
                [
                    game.currentScore,
                    game.bestScore,
                    game.bestBlock,
            ...
            connection.query(
                "UPDATE current_board SET wholeBoard=? WHERE user_id=?",
                [JSON.stringify(game.wholeBoard), req.cookies.user_id]
            );
...
function game_over(req, res, game, killerPiece) {
    ...
    connection.query(
        `INSERT INTO board_history (user_id, game_id, wholeBoard, score, startTime, endTime)
		VALUES (?, ?, ?, ?, ?, ?)`,
        [
            req.cookies.user_id,
            game.game_id,
            JSON.stringify(game.wholeBoard),
            game.currentScore,
            game.startTime,
            new Date(),
        ],
        async (err) => {
            ...
            await Promise.all([
                new Promise((resolve) => {
                    connection.query(
                        `UPDATE game SET currentScore=?, bestBlock=?, averageScore=?,
				totalGames=?, wins=(SELECT wins FROM game WHERE user_id=?)+?,
				${
                    killerPiece
                        ? `killedBy${killerPiece}=(SELECT killedBy${killerPiece} FROM
					game WHERE user_id=?)+1 `
                        : `giveUps=(SELECT giveUps FROM
					game WHERE user_id=?)+1`
                } WHERE user_id=?`,
                        [
                            0,
                ...
                new Promise((resolve) => {
                    connection.query(
                        `UPDATE current_board SET game_id=?, wholeBoard=?, startTime=?
						WHERE user_id=?`,
                        [uuidv4(), null, null, req.cookies.user_id],
                        resolve
                    );
                }),
            ]);
...
app.post("/game-over", loggedIn, (req, res, next) => {
    ...
    if (!allowed_new_blocks[killerPiece] && killerPiece != 0)
        return res.send("2");
...
app.get("/username-available/:username", (req, res, next) => {
    ...
    if (!username || !username.length) return res.send("0");
    ...
    connection.query(
        ...
        (err, is_user) => {
            ...
            if (is_user.length) return res.send("0");
...
app.get("/email-available/:email", (req, res, next) => {
    ...
    if (!email || !email.length) return res.send("0");
...
function signup_valid(body) {
    ...
    if (!hard_email_validate(body.email)) invalid_response += "0,";

    if (!body.username || !body.username.length || body.username.includes("@"))
        invalid_response += "1,";

    if (!body.password || !body.password.length) invalid_response += "2,";
...
app.post("/signup", async (req, res, next) => {
    ...
    connection.query(
        "INSERT INTO user (username, email, password, joindate) VALUES(?, ?, ?, NOW());",
        [req.body.username, req.body.email, encryptPass],
        (err) => {
            if (err) {
                return next({
                    err,
            ...
            connection.query(
                "SELECT id FROM user WHERE username=? AND email=?",
                ...
                async (err, newUserID) => {
                    ...
                    await new Promise((resolve) => {
                        connection.query(
                            "INSERT INTO streak (user_id, lastLogin, currentStreak, bestStreak) VALUES (?, ?, 1, 1)",
                            [u_id, new Date()],
                            (err) => {
                                if (err) {
                                    return next({
                    ...
                    await new Promise((resolve, reject) => {
                        connection.query(
                            "INSERT INTO current_board (user_id, startTime, game_id) VALUES (?, ?, ?)",
                            [u_id, new Date(), game_id],
                            (err) => {
                                if (err) {
                                    return next({
                                        err,
                    ...
                    connection.query(
                        "INSERT INTO auth (user_id, authToken, tokenDeath) VALUES (?, ?, ?);",
                        [u_id, newUUID, 2629800000],
                        (err) => {
                            if (err) {
                                return next({
                                    err,
...
function streakUpdate(user) {
    return new Promise((resolve, reject) => {
        ...
        if (
            currentDate.getMilliseconds() - lastLoginDate.getMilliseconds() <
                2629800000 &&
            dateDiff > 1 &&
            dateDiff < 2
        ) {
            connection.query(
                "UPDATE streak SET lastLogin=?, currentStreak=?, bestStreak=? WHERE user_id=?",
                [
                    currentDate,
                    user.currentStreak + 1,
                    user.currentStreak + 1 > user.bestStreak
                        ? user.currentStreak + 1
                        : user.bestStreak,
                    user.id,
                ],
            ...
            connection.query(
                "UPDATE streak SET lastLogin=?, currentStreak=? WHERE user_id=?",
                [currentDate, dateDiff <= 1 ? user.currentStreak : 1, user.id],
                (err) => {
                    if (err) return reject(err);
...
app.post("/login", (req, res, next) => {
    if (!req.body.username_email || !req.body.password) {
        res.send(
            "1-" +
                (req.body.username_email ? "" : "0,") +
                (req.body.password ? "" : "1,")
        );
        return;
    }

    let email_or_username = req.body.username_email.includes("@")
        ? "email"
        : "username";
    connection.query(
        `SELECT id, username, password, lastLogin, currentStreak, bestStreak, bestScore, currentScore, wholeBoard, authToken, tokenDeath FROM user INNER JOIN
		...
		INNER JOIN auth ON user.id=auth.user_id WHERE ${email_or_username}=?`,
        req.body.username_email,
        (err, user_password) => {
            ...
            bcrypt.compare(
                ...
                function (err, result) {
                    ...
                    let newUUID =
                        user_password[0].tokenDeath > 0
                            ? user_password[0].authToken
                            : uuidv4();

                    connection.query(
                        "INSERT INTO auth (user_id, authToken) VALUES (?, ?) ON DUPLICATE KEY UPDATE authToken=?, tokenDeath=2629800000;",
                        [user_password[0].id, newUUID, newUUID],
                        async (err) => {
                            if (err) {
                                return next({
                                    err,
...
app.post("/recover", (req, res, next) => {
    ...
    let email_or_username = req.body.username_email.includes("@")
        ...
        : "username";
    connection.query(
        `SELECT id, email FROM user WHERE ${email_or_username}=?`,
        req.body.username_email,
        (err, user_password) => {
            if (err || !user_password) {
                return next({
            ...
            let nonce = uuidv4();
            connection.query(
                "UPDATE user SET recovery_nonce=? WHERE user_id=?",
                [nonce, user_password[0].id],
                async (err) => {
                    if (err) {
                        return next({
                            err,
